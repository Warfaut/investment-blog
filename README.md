# 📘 Investment & Business Analytics Blog

Welcome to my personal workspace for investment research, financial analysis, and Python-based analytics.

This repository serves as a structured collection of:
- 📈 **Company analyses**
- 📊 **Market & sector research**
- 📐 **Valuation notes & corporate finance concepts**
- 🐍 **Python notebooks for portfolio and risk analytics**

---

### 🔍 **Company Analyses**

Deep dives into public companies, including:
- Business model overview
- Revenue & margin analysis
- Competitive landscape
- Growth drivers and risks
- Valuation (DCF + multiples)
- Investment thesis

📁 Folder: [/company-analyses](/company-analyses)

- [Adyen – Financial Overview (H2 2023 → H1 2025)](Adyen/company-analyses/adyen_financial_overview.md)

---

### 🌍 **Market & Sector Research**
Industry breakdowns, TAM/SAM/SOM estimates, macro trends, and emerging opportunities.

📁 Folder: [/market-research](/market-research)

---

### 📐 **Valuation Notes**
Short, practical explanations of:
- WACC  
- CAPM & beta  
- DCF model logic  
- Multiples and comparables  
- Capital structure decisions  

📁 Folder: [/valuation-notes](/valuation-notes)

---

### 🐍 **Python Analytics**
Python notebooks for:
- Portfolio risk modelling (VaR, CVaR, volatility)
- Financial data pipelines
- Time series analysis  
- Backtesting simple strategies

📁 Folder: [/python-analytics](/python-analytics)

---

## 🎯 Purpose of This Blog

This project helps me:
- Improve investment research and valuation skills  
- Develop a consistent corporate finance framework  
- Build a portfolio of analytical work for employers  
- Practice Python for financial and market data  
- Communicate insights clearly and professionally  

Everything here evolves as I learn.

---

## 📫 Contact  
**LinkedIn:** https://www.linkedin.com/in/enzub  
**Email:** enzubkoff@gmail.com
